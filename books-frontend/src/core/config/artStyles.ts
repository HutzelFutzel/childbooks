 ...
 * document only attaches an example image to a preset by id. The setup wizard
 * shows the image when present and falls back to the gradient swatch otherwise.
 */
import { z } from "zod";
...
export function normalizeArtStylesConfig(input: unknown): ArtStylesConfig {
  ...
  return { version: 1, examples: stored.examples ?? {} };
}
