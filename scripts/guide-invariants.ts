...
const memory = checkGuideMemory();
failures.push(...memory.failures);
notes.push(...memory.notes);
cases += memory.cases;

const admin = checkGuideAdmin();
failures.push(...admin.failures);
...
console.log();
for (const line of memory.report) console.log(line);
